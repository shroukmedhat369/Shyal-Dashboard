    ...
    <div class="content">
      <div class="driver-details">
        <div class="btns">
          ...
          <button @click="showDragpage" class="btn">Documents</button>
        </div>
        <div style="display: flex;" class="driver-details-container">
          <div style="margin-left: 15px;">
            <div class="driver" v-if="driver">
              <div class="driver-info">
                <img src="../assets/profile.png" alt="Driver Photo">
                <p><strong>Name:</strong> {{ driver.driverName }}</p>
                <p><strong>Email:</strong> {{ driver.driverEmail }}</p>
                <p><strong>Phone Number:</strong> {{ driver.driverPhoneNumber }}</p>
              </div>
          ...
          <div>
            <div >
              ...
              <div v-if="showDrag">
                <div class="outdiv">
                  <div class="drag-drop-area" :class="{ 'drag-over': isDragging }" @dragover.prevent="handleDragOver" @dragleave="handleDragLeave" @drop.prevent="handleDrop">
                    <p v-if="!droppedFiles.length">Drag and drop your files here</p>
                    <ul v-else>
                      <li v-for="(file, index) in droppedFiles" :key="index">{{ file.name }}</li>
                    </ul>
                  </div>
                </div>
                <button @click="saveToDatabase" :disabled="droppedFiles.length === 0" class="upgrade-btn">Upload</button>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async fetchDriverDetails() {
      try {
        ...
        this.driver = response.data[0]; // Assuming the API returns an array with a single object
      } catch (error) {
    ...
    async fetchDriverOrders() {
      try {
        const response = await axios.get(`https://localhost:44330/api/Driver/${this.driverId}/orders`);
        console.log(response.data); // Log the response data
  ...
  try {
    const response = await axios.get(`https://localhost:44330/api/Driver/${this.driverId}/pending-orders-Number`);
    console.log(response.data); // Log the response data
...
async fetchLastOrderTime() {
  try {
    const response = await axios.get(`https://localhost:44330/api/Driver/last-order-time/${this.driverId}`);
    console.log(response.data); // Log the response data
    ...
    async saveToDatabase() {
      ...
      try {
        const response = await axios.post(`https://localhost:44330/api/Driver/upload-document/${this.driverId}`, formData, {
          headers: {
